        ...
        <form action="{{route('backend.admins.store')}}" method="post" enctype="multipart/form-data">
            ...
            <div class="card flex-row-fluid mb-2  ">
                <div class="card-header">
                    <h3 class="card-title"> {{trans('backend.admin.create_new_admin')}}</h3>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="  mb-10">
                                <label for="password"
                                       class="required form-label">{{trans('backend.admin.password')}}</label>
                        ...
                        <div class="col">
                            <div class=" mb-10">
                                <label for="password_confirmation"
                                       class="required form-label">{{trans('backend.admin.password_confirmation')}}</label>
                    ...
                    <div class="row  align-items-center mb-10">
                        <div class="col-12 col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="role" data-control="select2"
                                        data-placeholder="Select an option">
                                    @foreach($roles as $role)
                                        <option {{ (old('role' ,'')== $role->id )?  'selected':""  }}    value="{{$role->id}}">{{$role->name}}</option>
                                    @endforeach
                                </select>
                                @error('role') <b class="text-danger"><i
                                            class="las la-exclamation-triangle"></i> {{$message}} </b> @enderror
