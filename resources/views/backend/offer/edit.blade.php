...
@section('content')
    <div class="col">
        <form action="{{route('backend.offers.update', ['offer'=> $offer->id])}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="card flex-row-fluid mb-2  ">
                ...
                <div class="card-body">
                        ...
                        <div class="row mb-10">
                            ...
                            <div class="col-lg-8 fv-row fv-plugins-icon-container">
                                ...
                                <input autocomplete="off" type="number" step="0.01" class="form-control values" id="from"
                                       name='from' value="{{old('from', $offer->from)}}"/>
                            ...
                            <div class="col-lg-8 fv-row fv-plugins-icon-container">
                                <div class="row">
                                    ...
                                    <div class="mb-0 col-12">
                                        <input class="form-control form-control values"
                                               id="to" name="to" type="number" step="0.01" min="1"
                                               value="{{old('to',$offer->to)}}"/>
                                    ...
                                    <div class="mb-0 col-2">
                                        <select class="form-control" id="discount_type" name="discount_type" required
                                            ...
                                            @foreach($discount_types as $type)
                                                <option value="{{$type}}"
                                                        {{old('discount_type',$offer->discount_type) == $type? "selected":"" }}>{{$type}}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="col-lg-8 fv-row fv-plugins-icon-container">
                                <select class="form-control" id="type" name="type" data-control="select2" required
                                    ...
                                    @foreach($types as $type)
                                        <option value="{{$type}}" {{old('type', $offer->type) == $type? "selected":""}}>{{$type}}</option>
                                    @endforeach
                                </select>
                    ...
                    <div id="product" class=" flex-row-fluid mb-2 @if(old('type', $offer->type) != 'Product')d-none @endif ">
                                    ...
                                    <div class="col-lg-8 fv-row fv-plugins-icon-container">
                                        <select class="form-control" id="products" name="products_ids[]"
                                            ...
                                            @foreach($products as $product)
                                                <option value="{{$product->id}}"@if(old('type', $offer->type) == 'Product')
                                                    {{in_array($product->id, old('products_ids', $offer->products_ids))? "selected":""}}
...
{{--                    <div id="order" class=" flex-row-fluid mb-2 @if(old('type', $offer->type) != 'Order')d-none @endif">--}}
...
{{--                                           id="minimum_shopping"--}}
{{--                                           name="minimum_shopping" step="0.01" value="{{old('minimum_shopping', $offer->minimum_shopping)}}"--}}
{{--                                    />--}}
                    ...
                    <div class="col">
                        <div class="mb-10">
                            <div class="form-check form-switch form-check-custom form-check-solid me-10">
                                <input class="form-check-input h-20px w-30px" @if(old('free_shipping', $offer->free_shipping)==1) checked
                        ...
                        <div class=" mb-10">
                            <div class="form-check form-switch form-check-custom form-check-solid me-10">
                                <input class="form-check-input h-20px w-30px" @if(old('status', $offer->status)==1) checked
                                       @endif type="checkbox" value="1"
