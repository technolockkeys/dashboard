<html lang="{{session('lang')}}" direction="{{session('lang') == 'ar'? 'rtl': 'ltr' }}"
      style="direction: {{session('lang') == 'ar'? 'rtl': 'ltr'}}">
                ...
                <div class="container-fluid d-flex align-items-stretch justify-content-between">
                    ...
                    <div class="d-flex">
                        ...
                        <div class="d-flex align-items-center ms-1 ms-lg-3">
                            <!--begin::Menu wrapper-->
                        @include('seller.layout.calculate_shipping')
                        @include('seller.layout.notifications')
                        <!--end::Menu wrapper-->
                        </div>
                            ...
                            <div class="d-flex align-items-stretch flex-shrink-0">
                                <div class="d-flex align-items-center ms-1 ms-lg-3" id="kt_header_user_menu_toggle">
                                    <div class="cursor-pointer symbol symbol-30px symbol-md-40px"
                                        ...
                                        <img
                                            src="{{auth('seller')->user()->avatar??asset('backend/media/avatars/blank.png')}}"
                                            alt="user"/>
                                    </div>
                                    <div
                                        ...
                                        <div class="menu-item px-3">
                                            <div class="menu-content d-flex align-items-center px-3">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <div
                                                        class="fw-bolder d-flex align-items-center fs-5">{{auth('seller')->user()->name}}
                                                    </div>
                                                    <a href="#"
                                                       class="fw-bold text-muted text-hover-primary fs-7">{{auth('seller')->user()->email}}</a>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="menu-item px-5" data-kt-menu-trigger="hover"
                                            ...
                                            <div class="menu-sub menu-sub-dropdown w-175px py-4">
                                                @foreach(get_languages() as $language)
                                                    <div class="menu-item px-3">
                                                        <a href="{{route('seller.set_locale',[ $language->code])}}"
...
<script>
    ...
    var messageFileManger = {
        'select_media': "{{trans('backend.global.select_media')}}",
        'click_to_select_files_here': "{{trans('backend.global.click_to_select_files_here')}}"
    }
    var csrf_token = "{{csrf_token()}}";
    var calculate_shipping_cost_route = '{{route('seller.products.calculate-shipping-cost')}}';
    var messages_translate = {
        'get_shipping_price': "{{trans('seller.get_shipping_price')}}",

    }
</script>
<script src="{{asset("backend/js/sellers/app.js")}}"></script>
@yield('script')
...
<script>
    ...
    $(document).on('click', '#get-quantity', function () {
        var product = $('#search-product-sku').val();

        var button = $('#get-quantity');
        button.html('<i class="fa fa-spinner fa-pulse fa-3x  fa-fw"></i>')
        button.attr('disabled', true)

        $.ajax({
            url: '{{route('seller.products.get-quantity')}}',
            method: "post",
            data: {
                "_token": '{{csrf_token()}}',
                'product_sku': product
                ...
                $('#product-search-error').text('');
                $('#display-quantity').removeClass('d-none')
                $("#product-quantity").val(response.data.quantity);
                setTimeout(function () {
                    $('#display-quantity').addClass('d-none');
                }, 2000);
                button.attr('disabled', false)
                button.html('{{trans('seller.get_quantity')}}')

            }, error: (response) => {
                button.attr('disabled', false)
                button.html('{{trans('seller.get_quantity')}}')
                $('#product-search-error').text(response.responseJSON.errors.product_sku);
