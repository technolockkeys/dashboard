...
@section('content')
    <div class="col">
        <form action="{{route('backend.languages.store')}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card flex-row-fluid mb-2  ">
                <div class="card-header">
                    <h3 class="card-title"> {{trans('backend.language.create_new_language')}}</h3>
                ...
                <div class="card-body">
                            ...
                            <div class=" mb-10">
                                ...
                                <select required name="code" id="code" class="form-control" data-control="select2">
                                    @foreach($codes as $code)
                                        <option @if(old('code') == strtolower($code) )  selected @endif value="{{strtolower($code)}}">{{strtolower($code)}}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row mb-10 flex-center">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="display_type" name="display_type"
                                    ...
                                    @foreach($display_types as $type)
                                        <option
                                            {{ (old('display_type' ,'')== $type? "selected":"" )}} value="{{$type}} ">{{$type}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col">
                            <div class="row mb-6">
                                <label for="flag required"
                                       class="col-lg-4 col-form-label fw-bold fs-6">{{trans('backend.language.flag')}} </label>
                                <div class="col-lg-8">


                                    {!! single_image('flag' , media_file(old('flag')), old('flag','') ) !!}
                                    <br>
