...
@section('title',trans('backend.menu.currencies').' | '.get_translatable_setting('system_name', app()->getLocale()))
    ...
    <div class="col">
        <div class="card   flex-row-fluid mb-2  ">
            <div class="card-header">
                <h3 class="card-title"> {{trans('backend.menu.currencies')}}</h3>
                ...
                <div class="table-responsive">
                    <table id="datatable" class="table table-rounded table-striped border gy-7 w-100 gs-7">
                        ...
                        <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
                            ...
                            <th>{{trans('backend.currency.name')}}</th>
                            <th>{{trans('backend.currency.code')}}</th>
                            <th>{{trans('backend.currency.value')}}</th>
                            <th>{{trans('backend.global.created_at')}}</th>
                            <th>{{trans('backend.global.updated_at')}} </th>
                            <th>{{trans('backend.global.status')}}</th>
                            <th>{{trans('backend.global.is_default')}}</th>
