<div class="row">
    ...
    <div class="col-12 col-md-4  text-left">
        <div class="form-group">
            ...
            <br>
            {!! single_image('twitter_image' , media_file(old('twitter_image') ), old('twitter_image')) !!}
            <br>
    ...
    <div class="col-12 col-md-4  text-left">
        <div class="form-group">
            ...
            <br>
            {!! single_image('pdf' , media_file(old('pdf') ), old('pdf') , 'pdf') !!}
            <br>
            @error('twitter_image')<b class="text-danger">{{ $message }}</b> @enderror
        </div>
    </div>
            ...
            @if(!empty(old('videos_provider[]',[])) )
                @foreach(old('videos_provider[]',[]) as $key=>$item)
                    <div class="row mt-3" data-row="{{$key}}">
                        ...
                        <div class="col-12 col-md-6">
                            <input name="video_url[]" value="{{!empty(old('video_url[]')[$key]) ? old('video_url[]')[$key] :""}}" type="text" class="form-control" required>
                        </div>
                        <div class="col-12 col-md-1 ">
                            <button type="button" onclick="remove_video('{{$key}}')" class="btn btn-icon btn-danger"> <i class="fa fa-exclamation-triangle"></i> </button>
                        </div>
                    </div>
