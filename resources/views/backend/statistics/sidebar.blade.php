@if(permission_can('show statistics' ,'admin') )
    <div
            class="menu-item  @if(request()->routeIs('backend.statistics.index')  ) show @endif ">
									<span class="menu-link">
										<span class="menu-icon">
<!--begin::Svg Icon | path: /var/www/preview.keenthemes.com/kt-products/docs/metronic/html/releases/2022-08-27-195308/core/html/src/media/icons/duotune/graphs/gra004.svg-->
<span class="svg-icon svg-icon-muted"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path opacity="0.3" d="M11 11H13C13.6 11 14 11.4 14 12V21H10V12C10 11.4 10.4 11 11 11ZM16 3V21H20V3C20 2.4 19.6 2 19 2H17C16.4 2 16 2.4 16 3Z" fill="currentColor"/>
<path d="M21 20H8V16C8 15.4 7.6 15 7 15H5C4.4 15 4 15.4 4 16V20H3C2.4 20 2 20.4 2 21C2 21.6 2.4 22 3 22H21C21.6 22 22 21.6 22 21C22 20.4 21.6 20 21 20Z" fill="currentColor"/>
</svg>
</span>
                                            <!--end::Svg Icon-->
                     					</span>
                                            <a class="menu-title"
                                               href="{{route('backend.statistics.index')}}">
                                                <span class="menu-title">{{trans('backend.menu.statistics')}}</span>
                                            </a>

									</span>

    </div>
@endif
