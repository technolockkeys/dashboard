<div class="row">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control has-error" name="title_{{$item->code}}" value="{{old('title_'.$item->code , $product->getTranslation ('title',$item->code) )}}"
    ...
    <div class="col-12 col-md-6">
        <div class="form-group">
            ...
            <input type="text" class="form-control" required id="sku" name="sku" value="{{old('sku' ,$product->sku)}}">
    ...
    <div class="col-12 col-md-6 ">
        <div class="form-group">
            ...
            <input type="number" step="2" class="form-control"   id="weight" name="weight" value="{{old('weight',$product->weight)}}">
            <b class="text-danger" id="error_weight"> @error('weight')<i class="fa fa-exclamation-triangle"></i> {{$message}}@enderror</b>
        </div>
    </div>
    ...
    <div class="col-12 col-md-6">
        ...
        <select name="type" id="type" class="form-control" data-control="select2">
            <option @if(old('software',$product->type) == 'software') selected @endif value="software">{{trans('backend.product.software')}}</option>
            <option @if(old('physical',$product->type) == 'physical') selected @endif value="physical">{{trans('backend.product.physical')}}</option>
        </select>
    </div>
    <div class="col-12 col-md-6">

        <label for="colors" class="">{{trans('backend.product.colors')}}</label>
        <select class="form-control"    id="color" name="color">
            <option @if(old('colors', $product->color_id ) == "") selected @endif value="">{{trans('backend.global.not_found')}}</option>
            @foreach($colors as $item)

                <option data-color="{{$item->code}}" @if($item->id=== old('colors', $product->color_id ) ) selected
                        @endif  value="{{$item->id}}">   {{$item->name}}</option>
            @endforeach
        </select>
      <b class="text-danger" id="error_colors">  @error('colors')<i class="fa fa-exclamation-triangle"></i> {{$message}}@enderror</b>
    </div>
