...
@section('content')
    <div class="col">
        <form action="{{route('backend.sellers.store')}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card flex-row-fluid mb-2  ">
                <div class="card-header">
                    <h3 class="card-title"> {{trans('backend.seller.create_new_seller')}}</h3>
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6" id="seller_manager_wrapper">
                            <div class="  mb-10">
                                ...
                                <select class="form-control" id="seller_manager" name="seller_manger"
                                        data-control="select2"
                                        data-placeholder="Select an option">
                                    <option value=""></option>
                                    @foreach($sellers as $seller)
                                        <option value="{{$seller->id}}" {{old('seller_manager')== $seller->id? 'selected':''}}>{{$seller->name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-md-6">
                            <div class=" mb-10">
                                <label for="seller_product_rate"
                                       class=" form-label required">{{trans('backend.seller.seller_product_rate')}}</label>

                                <div class="mb-0 col-12">
                                    <input class="form-control form-control"
                                           id="seller_product_rate" name="seller_product_rate" type="number"
                                           step="0.001" min="0" max="100"
                                           value="{{old('seller_product_rate')}}"/>
                            ...
                            <div class=" mb-10">
                                ...
                                <select name="role" id="role" class="form-control" data-control="select2">
                                    @foreach($roles as $item)
                                        <option {{ (old('role')== $item->id )  }} value="{{$item->id}}">{{$item->name}}</option>
                                    @endforeach
                                </select>
                                @error('password_confirmation') <b class="text-danger"><i
                                            class="las la-exclamation-triangle"></i> {{$message}} </b> @enderror
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-12 col-sm-6  align-items-center">
                            <div class="form-group  align-items-center">
                                ...
                                <br>
                                <div class="form-check form-switch form-check-custom form-check-solid me-10">
                                    <input class="form-check-input h-20px w-30px" @if(old('manager')== 1) checked
